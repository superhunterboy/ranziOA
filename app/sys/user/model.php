...
class userModel extends model
{
    ...
    public function getDeptPairs($params = '', $dept = 0)
    {
    ...
    /**
     * Get parent moderators.
     *
     */
    public function getParentCategoryByAccount($account)
    {
        $userCategory = $this->dao->select('t1.account, t2.parent')->from(TABLE_USER)->alias('t1')
            ->leftJoin(TABLE_CATEGORY)->alias('t2')->on('t1.dept=t2.id')
        ...
        {
            $parentCategory = $this->dao->select('moderators')->from(TABLE_CATEGORY)->Where('id')->eq($userCategory->parent)->fetch();
            return trim($parentCategory->moderators, ',');
        }
        else
        {
            $parentCategory = $this->dao->select('moderators')->from(TABLE_CATEGORY)->Where('name')->eq($this->config->leave->moderatorsCategoryName)->fetch();
            return trim($parentCategory->moderators, ',');
        }
    }
...
//
//            $info = LDAP::login($account,$password);
//
...
//            }
//            //if($user->dept == '0') {
//            $user = $this->checkUserDept($user, $info);
//           // }
//        }
//        else {
            $user = $this->identify($account, $password);
//        }
    ...
    private function checkUserDept($user, $data) {
        ...
        $u = new stdClass();
        $u->dept = $user->dept;
        $this->dao->update(TABLE_USER)->data($u)->where('id')->eq($user->id)->exec();
        return $user;
    }

    public function getUserDept($user) {
        $dept = $this->dao->select('name')->from(TABLE_CATEGORY)->where('id')->eq($user->dept)->fetch();
    ...
    private function createUser($data) {
        $user = new stdclass();
        $user->account = strtolower($data['name']);

        $user->gender = 'u';
        $user->realname = $data['name'];
        $user->email = $data['email'];//str_replace('nyjt88.com','ay033.com', $data['email']);
        $user->join = date('Y-m-d H:i:s');
        $dept = $this->dao->select('id')->from(TABLE_CATEGORY)->where('name')->eq($data['dept'])->fetch();
        if($dept) $user->dept = $dept->id;

        $user->password = $this->createPassword( $data['password'], $user->account );
        ...
        if($id && $id->id) {
            ...
            $g->account = $data['name'];
            $g->group = 5;
            $this->dao->insert(TABLE_USERGROUP)->data($g)->exec();
    ...
    public function importInOut() {
        $data = $this->dao->select('*')->from('vw_inout')->orderby('CHECKTIME')->fetchall();
        if($data) {
            //$row = $data;
            foreach($data as $row) {
            //echo $row->CHECKTIME;
            ...
            if($attend && $attend->id) {
                ...
                if($this->caculateTime($attend, $datetime))
                {
                    $att->signOut  = date('H:i:s', $datetime);
                    $this->dao->update(TABLE_ATTEND)->data($att)->where('id')->eq($attend->id)->exec();
                }
            }
            else {
                $attend = new stdclass();
    ...
    private function caculateTime($attend, $time) {
    ...
    public function all() {
        //$info = LDAP::login('goslin','winson83');
        $info = LDAP::all();
        //print_r($info);

        foreach($info as $user) {
            //echo TABLE_USERGROUP;
            $dept = $this->dao->select('id')->from(TABLE_CATEGORY)->where('name')->eq($user['dept'])->fetch();

            if($dept && $dept->id) {
                $u = new stdClass();
                $u->dept = $dept->id;
                $this->dao->update(TABLE_USER)->data($u)->where('account')->eq($user['name'])->andWhere('dept')->eq('0')->exec();

                echo '<p>update:'.$user['name'].' dept:'.$dept->id.'</p>';
            }
