...
class user extends control
{
    ...
    public function ldap() {
        ...
        if(!empty($_POST))
        {
            ...
            if($user) {
                ...
                $data['dept'] = $this->user->getUserDept($user);
                echo json_encode(['code'=>'0','user'=>$data], JSON_UNESCAPED_UNICODE);
                exit;
            }

        }
        echo json_encode(['code'=>'404','user'=>[]]);
        exit;
    }
    ...
    public function login($referer = '')
    {
        ...
        if(!empty($_POST))
        {
            ...
            if(empty($blocks))
            {
                $this->loadModel('block')->initBlock('sys');
            }
            ...
            if($this->post->referer != false and strpos($loginLink . $denyLink, $this->post->referer) === false)
            {
                ...
                {
                    $this->send(array('result'=>'success', 'locate' => '/oa/dashboard/'));
                    //$this->send(array('result'=>'success', 'locate' => $this->createLink('oa', 'dashboard')));
                }
                else
                {
                    $this->send(array('result'=>'success', 'locate' => '/oa/dashboard/'));
    ...
    public function colleague($deptID = 0, $search = '', $orderBy = 'id_asc', $recTotal = 0, $recPerPage = 10, $pageID = 1)
    {
        ...
        }//die($this->locate(inlink('colleague', "deptID=$deptID&query={$this->post->query}&orderBy=$orderBy&recTotal=0&recPerPage=$recPerPage&pageID=1")));
