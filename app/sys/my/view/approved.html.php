...
<form method="post" action="" id="searchform" class="form-condensed">
    <table class="table table-condensed table-form" style="max-width: 1200px; margin: 0 auto">
        <tbody><tr>
            <td class="w-40px">
                <strong>日期</strong></span></td><td class="w-100px">
                            <select name="date" id="date" class="form-control">
                               <?php foreach ($dates as $date):?>
                                   <option value="<?php echo $date['v'] ?>" <?php echo $date['selected']?'selected':''?>><?php echo $date['txt'] ?></option>
                                <?php endforeach;?>
                            </select>
            </td>
            <td class="w-400px">
...
<?php $appendClass = $type == 'approved' ? 'table-noFixedHeader' : 'table-fixedHeader'?>
<?php if(($type == 'approved' && !empty($attends)) || $type == 'attend'):?>
<div class='panel'>
  <table class='table table-hover table-striped table-sorter table-data table-fixed text-center <?php echo $appendClass?>'>
    <thead>
      <tr class='text-center'>
        ...
        <th class='w-100px'><?php echo $lang->attend->reviewedBy;?></th>
          <th class='w-100px'><?php echo $lang->attend->reviewedDate;?></th>
      </tr>
    </thead>
    ...
    <tr>
      ...
      <td>
        <?php echo $attend->reviewedBy;?>
      </td>
        <td><?php echo $attend->reviewedDate;?></td>
    </tr>
    <?php endforeach;?>
  </table>
...
<?php if(($type == 'approved' && !empty($refunds)) || $type == 'refund'):?>
<div class='panel'>
  <table class='table table-hover table-striped table-sorter table-data table-fixed text-center <?php echo $appendClass?>'>
    <thead>
      <tr class='text-center'>
    ...
    <tr>
      ...
      <td><?php echo $refund->reviewedBy;?></td>
        <td><?php echo $refund->reviewedDate;?></td>
    </tr>
    <?php endforeach;?>
  </table>
  <?php if(!$refunds):?>
...
<?php if($type == 'approved' && (!empty($leaveList) or !empty($makeupList) or !empty($overtimeList) or !empty($lieuList))):?>
<div class='panel'>
  <table class='table table-data table-hover text-center table-fixed tablesorter <?php echo $appendClass?>' id='leaveTable'>
    <thead>
      <tr class='text-center'>
    ...
    <tr>
      ...
      <?php echo zget($lang->leave->countryList, $leave->country).' '.$leave->desc;?>
      </td>
      <td class='leave-<?php echo $leave->status?>'><?php echo zget($this->lang->leave->statusList, $leave->status);?></td>
      <td>
        <?php echo $leave->reviewedBy;?>
      </td>
        <td><?php echo $leave->reviewedDate;?></td>
    </tr>
    <?php endforeach;?>
    ...
    <tr>
      ...
      <td>
          <?php echo $makeup->reviewedBy;?>
      </td><td><?php echo $makeup->reviewedDate;?></td>
    </tr>
    <?php endforeach;?>
