...
class attendModel extends model
{
    ...
    public function getAttendByManualEdited($account, $startDate = '', $endDate = '')
    {
        $attends = $this->dao->select('COUNT(*) nums')->from(TABLE_ATTEND)
            ->where('account')->eq($account)
            ->andWhere('reviewStatus')->in(array('pass','wait'))
            ->beginIf($startDate != '')->andWhere('`date`')->ge($startDate)->fi()
            ->beginIf($endDate != '')->andWhere('`date`')->le($endDate)->fi()
            ->fetch();
    ...
    public function getApprovedAttends($deptID='',  $year = '', $month = '', $orderBy='')
    {
        $date = '';
        if($year)
        {
        ...
        return $this->dao->select('*')->from(TABLE_ATTEND)
            ->where('reviewStatus')->eq('pass')
            ->beginIf($date)
            ->andWhere('date', true)->like($date)
...
*/
                    $time = strtotime($attend->signOut) - strtotime($attend->signIn);
                    if($time<0) {
                        $desc = $this->lang->attend->statusList['none'];
                    }
                    else if($time > ($this->config->attend->workingHours * 3600)) {
                        $desc = $this->lang->attend->statusList['normal'];
                    }
                    else {
                        $desc = $this->computeDesc($time);
                    }
                    $attend->desc = $desc;
...
EOT;
    ...
    public function review($attendID, $status)
    {
        ...
        if($status == 'pass')
        {
            ...
            $this->dao->update(TABLE_ATTEND)
                ...
                ->set('reviewedDate')->eq(date('Y-m-d H:i:s'))
        ...
        if($status == 'reject')
        {
            $this->dao->update(TABLE_ATTEND)->set('reviewStatus')->eq('reject')->set('reviewedBy')->eq($this->app->user->account)->set('reviewedDate')->eq(date('Y-m-d H:i:s'))->where('id')->eq($attendID)->exec();
        }
    ...
    /**
     * Update status of unknown status attend.
     * 
     * @access public
     * @return bool
     */
    ...
    public function computeStatus($attend) {
        ...
        if(($signOut-$signIn)<$work) $status = 'early';

        if($this->isRestDay($attend->date)) $status = 'rest';

        return $status;
    }
    public function computeStatus2($attend)
    {
    ...
    /**
     * Process attend, add dayName, compute today's status.
     * 
     * @param  object $attend 
     * @access public
     * @return object
    ...
    public function setManager()
    {
        ...
        foreach($deptList as $id => $dept)
        {
            if(!empty($dept))
            {
                //更新sys_user的直属上级supervisor
                $this->dao->update('sys_user')->set('supervisor')->eq($dept)->where('dept')->eq($id)->exec();
