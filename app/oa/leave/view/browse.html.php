...
<?php $batchReview = $type == 'browseReview' && commonModel::hasPriv('leave', 'batchReview');?>
    <div class='panel'>
      ...
      <table class='table table-hover text-center table-fixed tablesorter' id='leaveTable'>
        ...
        <?php if($type == 'browseReview' && $leave->createdBy == $this->app->user->account):continue;endif;?>
        ...
        <?php endif?>
          ...
          <td class='actionTD text-left'>
            ...
            else
            {
                ...
                {
                    commonModel::printLink('oa.leave', 'edit',   "id={$leave->id}",               $lang->edit,                        "data-toggle='modal'");
                    if($leave->status == 'doing' && !$reviewedByMe)
                    {
                    ...
                    else
                    {
                        if($type == 'company')
                        {
                            if($leave->createdBy == $this->app->user->account ||  $this->app->user->admin == 'super')
                            {
                                echo html::a('###', $lang->leave->statusList['pass'],   "disabled='disabled'");
                                echo html::a('###', $lang->leave->statusList['reject'], "disabled='disabled'");
                            }
