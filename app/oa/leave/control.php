...
class leave extends control
{
    ...
    public function browse($type = 'personal', $date = '', $orderBy = 'id_desc')
    {
        ...
        elseif($type == 'browseReview')
        {
            ...
            elseif($reviewedBy && $reviewedBy == $this->app->user->account)
            {
                $reviewedByMe = true;
                $leaveList = $this->leave->getList($type, $currentYear, $currentMonth, '', '', 'doing', $orderBy);
            }
            else
            {
        ...
        {
            $managedDepts = $this->loadModel('tree')->getDeptManagedByMe($this->app->user->account);
            $dept = array_keys($managedDepts);
            if($this->app->user->admin == 'super')
            {
                $dept = [$this->app->user->dept];
            }
            $leaveList = $this->leave->getList($type, $currentYear, $currentMonth, '', $dept, '', $orderBy);
        }

        $this->view->reviewedByMe   = $reviewedByMe;
    ...
    public function view($leaveID, $type = '')
    {
        $reviewedByMe = false;
        $reviewedBy = $this->leave->getReviewedBy();
        if($reviewedBy && $reviewedBy == $this->app->user->account)
        {
            $reviewedByMe = true;
        }
        $this->view->reviewedByMe = $reviewedByMe;
    ...
    public function review($id, $status, $mode = '')
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if($this->app->user->account == $createdUser->supervisor && $this->app->user->account != $leave->createdBy) $canReview = true;
            }
        }
        ...
        if($status == 'pass')
        {
            ...
            {
                if(isset($reviewedBy) && $reviewedBy != $this->app->user->account && $leave->country == $this->config->leave->country) $status = 'doing';
                $this->leave->review($id, $status, $secondReviewer);
                if(dao::isError()) $this->send(array('result' => 'fail', 'message' => dao::getError()));
