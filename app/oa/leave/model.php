...
class leaveModel extends model
{
    ...
    public function getList($type = 'personal', $year = '', $month = '', $account = '', $dept = '', $status = '', $orderBy = 'id_desc')
    {
        ...
        $leaveList = $this->dao->select('t1.*, t2.realname, t2.dept, t2.supervisor')
            ...
            ->beginIf($type == 'browseReview')->andWhere('t1.status')->in('wait,back,doing')->orWhere('t2.supervisor')->eq($this->app->user->account)->andWhere('t1.status')->in('wait,back')->fi()
            ->beginIf($type == 'approved')->andWhere('t1.status')->in('pass,reject')->fi()
            ->beginIf($type == 'company')->andWhere('t1.status')->ne('draft')->orWhere('t2.supervisor')->eq($this->app->user->account)->fi()
    ...
    public function processStatus($leaveList)
    {
        ...
        {
            $reviewer = $leave->secondReviewer ? $leave->secondReviewer : $leave->supervisor;

            if('doing' == $leave->status)   //如果需要二次审核
            {
                $reviewer = $this->getReviewedBy();
                $reviewer .= '二次';
            }
            $leave->statusLabel = $reviewer. zget($this->lang->leave->statusList, $leave->status);
        }
    ...
    {
        return !isset($this->config->leave->reviewedBy) ? (!isset($this->config->attends->reviewedBy) ? '' : $this->config->attends->reviewedBy) : $this->config->leave->reviewedBy;
    }
     ...
     * @param  int    $id
     * @param  string $status
     * @param  string $secondReviewer
     * @access public
     * @return bool
     */
    public function review($id, $status, $secondReviewer=NULL)
    {
        ...
        if($secondReviewer)
        {
            $data->secondReviewer     = $secondReviewer;
            $data->secondReviewerDate = helper::now();
        }
        else
        {
